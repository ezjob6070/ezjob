...
const EditTechnicianModal: React.FC<EditTechnicianModalProps> = ({
  ...
  const form = useForm<TechnicianEditFormValues>({
    ...
    defaultValues: {
      ...
      status: (technician?.status === "on_leave" ? "onLeave" : (technician?.status as "active" | "inactive" | "onLeave")) || "active",
      ...
      salaryBasis: (technician?.salaryBasis === "per-project" ? "hourly" : technician?.salaryBasis) as any || "hourly",
