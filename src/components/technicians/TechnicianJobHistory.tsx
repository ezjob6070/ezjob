...
type TechnicianJobHistoryProps = {
  technician: Technician;
};

const TechnicianJobHistory: React.FC<TechnicianJobHistoryProps> = ({ technician }) => {
  // Filter jobs for this technician
  const technicianJobs = initialJobs.filter(
    (job) => job.technicianName === technician.name
  );

  // Sort jobs by date (most recent first)
  const sortedJobs = [...technicianJobs].sort(
  ...
  if (sortedJobs.length === 0) {
    return (
      <div className="text-center py-8">
        <p className="text-muted-foreground">No job history available for this technician.</p>
      </div>
    );
  }
